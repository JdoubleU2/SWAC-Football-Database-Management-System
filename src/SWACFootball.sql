...
-- 2. Record or Update Player Stats Procedure
...
-- 3. Aggregate Team Stats Procedure
...
-- 4. Update Team Stats Table from PlayerStats Procedure
...
DELIMITER //
CREATE PROCEDURE CursorAllCoaches()
BEGIN
  DECLARE done INT DEFAULT FALSE;
  DECLARE coach_name VARCHAR(100);
  DECLARE team_id INT;
  DECLARE cur_coach CURSOR FOR SELECT Name, TeamID FROM Coach;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

  OPEN cur_coach;
  read_loop: LOOP
    FETCH cur_coach INTO coach_name, team_id;
    IF done THEN
      LEAVE read_loop;
    END IF;
    SELECT coach_name AS CoachName, team_id AS TeamID;
  END LOOP;
  CLOSE cur_coach;
END//
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE CursorPrairieViewPlayers()
BEGIN
  DECLARE done INT DEFAULT FALSE;
  DECLARE player_name VARCHAR(100);
  DECLARE player_position VARCHAR(50);
  DECLARE cur_player CURSOR FOR 
    SELECT p.Name, p.Position
    FROM Player p JOIN Team t ON p.TeamID = t.TeamID
    WHERE t.Name = 'Prairie View A&M';
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

  OPEN cur_player;
  read_loop: LOOP
    FETCH cur_player INTO player_name, player_position;
    IF done THEN
      LEAVE read_loop;
    END IF;
    SELECT player_name AS Name, player_position AS Position;
  END LOOP;
  CLOSE cur_player;
END//
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE CursorGames2025()
BEGIN
  DECLARE done INT DEFAULT FALSE;
  DECLARE g_date DATE;
  DECLARE home_team INT;
  DECLARE away_team INT;
  DECLARE cur_game CURSOR FOR SELECT Date, HomeTeamID, AwayTeamID FROM Game WHERE SeasonYear = 2025;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

  OPEN cur_game;
  read_loop: LOOP
    FETCH cur_game INTO g_date, home_team, away_team;
    IF done THEN
      LEAVE read_loop;
    END IF;
    SELECT g_date AS Date, home_team AS HomeTeamID, away_team AS AwayTeamID;
  END LOOP;
  CLOSE cur_game;
END//
DELIMITER ;
